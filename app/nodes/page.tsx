...
import { defaultConfig, nodeCatalog, nodeSpec, type NodeKind, type NodeSocket, type NodeSpec } from "@/lib/node-catalog";
import "@xyflow/react/dist/style.css";

type FlowData = { kind: NodeKind; title: string; subtitle: string; inputs: NodeSocket[]; outputs: NodeSocket[]; config: Record<string, string>; [key: string]: unknown };
type FlowSnapshot = { nodes: Node<FlowData>[]; edges: Edge[] };
...
const previewEndpoint = () => {
  return "/api/node-flows/node-lab";
};
...
const nodeCode = (item: Node<FlowData>) => {
  ...
  if (item.data.kind === "response") return `return json(records, { status: ${config.status} })`;
...
const node = (id: string, kind: NodeKind, x: number, y: number): Node<FlowData> => {
  const spec = nodeSpec(kind);
  return { id, type: "geoNode", position: { x, y }, data: { kind: spec.kind, title: spec.title, subtitle: spec.example, inputs: spec.inputs, outputs: spec.outputs, config: defaultConfig(kind) } };
};
const configuredNode = (id: string, kind: NodeKind, x: number, y: number, config: Record<string, string>) => {
  const created = node(id, kind, x, y);
  return { ...created, data: { ...created.data, config: { ...created.data.config, ...config } } };
};
...
const starterEdges: Edge[] = [{ id: "e1", source: "request-1", sourceHandle: "request", target: "resource-1", targetHandle: "request" }, { id: "e2", source: "resource-1", sourceHandle: "records", target: "filter-1", targetHandle: "records" }, { id: "e3", source: "filter-1", sourceHandle: "records", target: "response-1", targetHandle: "records" }];
...
function GeoNode({ id, data, selected }: NodeProps<Node<FlowData>>) {
  ...
  function updateConfig(key: string, value: string) {
    ...
    window.dispatchEvent(new CustomEvent("mockbase-flow-dirty"));
  }
  ...
  return <div className={`geo-node ${data.kind} ${selected ? "selected" : ""}`}>
    <div className="geo-node-header"><span>{data.kind}</span><strong>{data.title}</strong>{data.kind === "response" && <em>{data.config.status} OK</em>}</div>
...
export default function NodesPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      const response = await fetch("/api/flows/node-lab", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ nodes, edges }) });
  ...
  function reset() { replaceFlow(starterNodes, starterEdges, null, "Starter flow tiklandi. Saqlashni unutmang."); }
  async function ensureDemoProject() {
    const project = await fetch("/api/projects/node-demo");
    if (!project.ok) {
      ...
      if (!created.ok && created.status !== 409) throw new Error("Demo project yaratilmadi.");
    }
    const records = await fetch("/api/mock/node-demo/posts").then((response) => response.ok ? response.json() : []);
    if (records.length) return;
    await fetch("/api/mock/node-demo/posts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: "Published post", published: true }) });
    await fetch("/api/mock/node-demo/posts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: "Draft post", published: false }) });
  }
  async function buildTemplate(nextNodes: Node<FlowData>[], nextEdges: Edge[], selectedNode: string, message: string) {
    try {
      ...
      replaceFlow(nextNodes, nextEdges, selectedNode, `${message} Demo project: node-demo.`);
      const saved = await fetch("/api/flows/node-lab", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ nodes: nextNodes, edges: nextEdges }) });
      setDirty(!saved.ok);
  ...
  function buildGetPostsTemplate() { void buildTemplate(getPostsNodes, getPostsEdges, "request-template", "GET /posts real ishlashga tayyor."); }
  function buildFilteredTemplate() { void buildTemplate(filteredPostsNodes, filteredPostsEdges, "filter-filtered", "Filterli GET /posts real ishlashga tayyor."); }
  function buildCreateTemplate() { void buildTemplate(createPostNodes, createPostEdges, "request-create", "POST /posts real ishlashga tayyor."); }
  ...
  async function runSavedFlow(nextNodes: Node<FlowData>[]) {
    ...
    const response = await fetch(previewEndpoint(), { method, headers: { "Content-Type": "application/json" }, body: method === "GET" ? undefined : JSON.stringify(body) });
  ...
  async function openPreviewTab() {
    ...
    window.open(previewEndpoint(), "_blank", "noopener,noreferrer");
